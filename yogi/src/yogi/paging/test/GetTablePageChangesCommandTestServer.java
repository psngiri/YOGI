...
public class GetTablePageChangesCommandTestServer extends TestCase{

	public GetTablePageChangesCommandTestServer() throws CommandException {
		super();
	...
	public void testGetPage() throws CommandException {
		...
		Change changesHistory1 = new Change(ChangeType.MOD, 12, 1, "ACT", "DEF", null, false, false);
		Change changesHistory2 = new Change(ChangeType.MOD, 12, 2, "BDL", "IJK", null, false, false);
		Change changesHistory3 = new Change(ChangeType.MOD, 14, 1, "ACT", "LMN", null, false, false);
		Change changesHistory4 = new Change(ChangeType.MOD, 14, 2, "BHM", "PQR", null, false, false);
		Change changesHistory5 = new Change(ChangeType.ADD, 10, 0, "", "", null, false, false);
		Change changesHistory6 = new Change(ChangeType.MOD, 10, 1, "", "TUV", null, false, false);
		Change changesHistory7 = new Change(ChangeType.MOD, 10, 2, "", "WXY", null, false, false);
		Change changesHistory8 = new Change(ChangeType.DEL, 13, 0, "", "", null, false, false);
		changesHistory.add(changesHistory1);
		changesHistory.add(changesHistory2);
		changesHistory.add(changesHistory3);
		changesHistory.add(changesHistory4);
		changesHistory.add(changesHistory5);
		changesHistory.add(changesHistory6);
		changesHistory.add(changesHistory7);
		changesHistory.add(changesHistory8);
		...
		/*GetTablePageChangesCommand getTablePageChangesCommand = new GetTablePageChangesCommand(0, );
		System.out.println("GETTABLEPAGECHANGES command being executed........");
		Pair<List<String[]>, List<String[]>> changesPair = CommandExecutor.get().execute(getTablePageChangesCommand);
		
		int i = 0;
		for(String[] actualData : changesPair.getFirst()) {
			for(int j = 0; j < actualData.length; j++) {
				System.out.println("Actual Data [" + i + "] : " + actualData[j]);
			}
			i++;
		}
		
		i = 0;
		for(String[] updatedData : changesPair.getSecond()) {
			for(int j = 0; j < updatedData.length; j++) {
				System.out.println("Updated Data [" + i + "] : " + updatedData[j]);
			}
			i++;
		}*/
		
		//ClearTablePageCommand<Recom> clearTablePageCommand = new ClearTablePageCommand<Recom>(0);
		//CommandExecutor.get().execute(clearTablePageCommand);
